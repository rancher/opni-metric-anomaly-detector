...
class MetricAnomalyDetector:
    ...
    def run(self, data):
        ...
        if (
            len(self.metric_xs) >= MIN_DATA_SIZE
        ):  ## which means there's a prediction for this datapoint
    ...
    def change_point_detection(self):
        ...
        if len(training_y) >= 2 * CPD_TRACE_BACK_TIME:
            cpd = rpt.Pelt(model="rbf").fit(
                np.array(training_y)
            )  ## change point detection. maybe volatility is another option?
            ...
            for l in reversed(change_locations):
                ...
                ):  # the change point should near to an anomaly?
                    logger.debug(
    ...
    def train_and_predict(self):
        ...
        if (
            len(self.metric_xs) >= MIN_DATA_SIZE
        ):  # and (len(self.metric_xs) - MIN_DATA_SIZE) % RETRAINING_INTERVAL_MINUTE == 0: ## TODO: better rule
